...
type TalkConversationPageProps = {
  params: Promise<{
    slug: string
...
export default async function TalkConversationPage({
  params
}: TalkConversationPageProps) {
  ...
  if (conversationId === 'new') {
    const conversation = await createConversation(serviceClient, {
      personaId: access.persona.id,
      userId: access.userId
    })
    redirect(`/talk/${access.persona.slug}/${conversation.id}`)
  }

  const conversation = await getConversationById(serviceClient, conversationId)

  if (
    !conversation ||
    conversation.persona_id !== access.persona.id ||
    conversation.user_id !== access.userId
  ) {
    notFound()
  }
