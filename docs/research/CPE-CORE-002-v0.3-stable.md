...
18. Latency tiers.
19. Hot memory cache.
20. Downstream use contracts for Reasoning, Identity, Embodiment, and Capture.
21. Example `MemoryBundle`.
22. Implementation notes for interim and target architectures.
23. Failure modes.
24. Open questions.
25. Summary.
...
`MemoryContent` represents the primary stored content of a memory record.

`MediaRef` is a reference to an asset stored in external media storage. In the interim implementation this is Backblaze B2.
...
`MemoryLifecycle` describes the operational state of a record.

It answers:

> Is this record active, hidden, archived, deleted, or under review?

`ConsolidationStatus` describes the epistemological state of a record.

It answers:

> How authoritative, durable, reviewed, or identity-relevant is this record?

These are independent dimensions.

A record may be `consolidated_identity_relevant` and `suppressed` simultaneously.
...
## 6.8 Emotional / Implicit Memory Record
...
interface ProvenanceProfile {
  ...
  provenanceConfidence: number; // 0.0 - 1.0
}
```
...
Consent may be withdrawn by the subject or their designated representative.

Withdrawal does not automatically delete records, but it must trigger governed review of affected memory records.
...
interface AuthorityProfile {
  ...
  authorityConfidence: number; // 0.0 - 1.0
...
2. Authorised estate representative with legal designation.
3. Trusted family reviewer with limited scope.
4. System governance process for technical contradictions or detected errors only.

A trusted family reviewer may challenge, contextualise, or supply additional evidence, but may not unilaterally replace high-authority historical memory.

No correction authority created post-death may increase the `authorityLevel` of a memory record beyond `trusted_witness`.

A correction made post-death must carry `memoryType: "correction"` and must include a `correctionAuthority` field identifying the post-death authority source.

Post-death correction must not silently rewrite subject-authored, subject-approved, or captured memory. It must create an auditable correction record.
...
interface MemoryConfidence {
  ...
  emotionalConfidence: number; // 0.0 - 1.0
  interpretationConfidence: number; // 0.0 - 1.0
...
interface RelationshipRef {
  ...
  emotionalCloseness?: number; // 0.0 - 1.0
  confidence: number; // 0.0 - 1.0
}
```
...
## 13.2 Relationship Graph Schema
...
The full graph schema, traversal algorithms, and graph storage requirements will be detailed in a named appendix or sub-specification.

This section defines the minimal graph objects sufficient for CPE-CORE-003 interface contracts.
...
`storageRef` points to the pgvector index entry.

---

## 14.2 Embedding Facets
...
interface RetrievalConfidence {
  ...
  topCandidateScore: number; // 0.0 - 1.0
  candidateSpread: number; // 0.0 - 1.0
  crossFacetCoherence: number; // 0.0 - 1.0
  authorityAlignment: number; // 0.0 - 1.0
  temporalCoherence: number; // 0.0 - 1.0
  emotionalCoherence: number; // 0.0 - 1.0
  contradictionPenalty: number; // 0.0 - 1.0
...
## 29.1 Promotion Workflow

The promotion pathway from `pending_consolidation` to `consolidated_identity_relevant` requires governed review.

The minimum workflow is:

1. A review initiator flags the interaction memory as a promotion candidate.

   The initiator may be:

   - the system;
   - the subject during capture;
...
4. The reviewer records one of the following decisions:
...
6. On partial acceptance, only the approved portion may be promoted. The unapproved portion remains `pending_consolidation`, `rejected`, or `quarantined` depending on review outcome.

7. On rejection, the record is moved to `rejected` consolidation status and must not influence Identity Layer parameters.

8. The original interaction memory record is preserved in all cases.

Promotion creates a governed copy or tagged amendment.

Promotion must not overwrite the original interaction memory.
...
The interim implementation uses a single embedding pipeline:
...
Embeddings are stored in a single pgvector index.

This represents a subset of the multi-facet embedding model specified in §14.

In the interim implementation, the `embeddingSet.semantic` facet is populated.

The following facets are deferred:
...
Retrieval queries against deferred facets must fall back to semantic search and graph traversal until the full embedding pipeline is implemented.

The `MemoryEmbeddingSet` schema remains valid.

Unpopulated facets are represented as absent optional fields.

---

## 34.1.1 Interim Schema Mapping

| Supabase Table | CPE-CORE-002 Concept | Notes |
|---|---|---|
| `diary_entries` | `BaseMemoryRecord` | `memoryType: "captured"`, `memorySystem: "episodic"` |
| `interview_sessions` | `BaseMemoryRecord` | `memoryType: "captured"`, `memorySystem: "semantic"` |
| `persona_traits` | `BaseMemoryRecord` | `memoryType: "derived"`, `memorySystem: "semantic"`; requires full provenance fields in target schema |
| `emotional_voice_samples` | `BaseMemoryRecord` | `memoryType: "captured"`, `memorySystem: "emotional_implicit"`; includes capture quality metadata |

The table uses canonical schema language rather than informal record subtype names. The target implementation will migrate or extend these tables to conform to the full `BaseMemoryRecord` schema.
...
4. How should the threshold for "reasonable inference from captured material" be operationalised in practice?
...
15. What tooling and UI is required to make the promotion review workflow accessible to non-technical reviewers? Deferred to CPE-CORE-007.
16. What legal or contractual mechanism should govern the designation of post-death correction authority? Refer to CPE-CORE-004 and legal/consent architecture.
...
The system stores memory records with provenance, authority, confidence, temporal context, emotional salience, relationship links, contradiction state, consolidation status, lifecycle state, and consent metadata.
