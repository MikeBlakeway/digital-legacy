...
if (!hasReactServerCondition()) {
  if (process.env[REEXEC_ENV] === '1') {
    console.error(
      'Missing react-server condition after re-exec; cannot import server-only wrappers.'
    )
    process.exit(1)
  }
...
async function main(): Promise<void> {
  ...
  await runTest('analyse emotion_classify', async () => {
    const emotion = await classifyEmotion(
      'I cannot believe he remembered that little detail.',
      {
        timeoutMs: ANALYSE_TIMEOUT_MS
      }
    )
  ...
  await runTest('tts', async () => {
    ...
    try {
      const response = await synthesizeSpeech(input, {
        timeoutMs: TEST_TIMEOUT_MS
      })
...
async function runTest(
  ...
  try {
    ...
    console.log(
      `✓ ${name} — ${message} (${formatElapsed(Date.now() - startedAt)})`
    )
  } catch (error) {
